...
t_Collision	collision_find_wall(t_mini_map *mini_map, t_character *player,
		double rayDir)
{
	...
	vertical_hit = collision_find_vertical_hit(mini_map, player, rayDir,
			direction);
	horizontal_hit = ray_find_horizontal_hit(mini_map, player, rayDir,
			direction);
	...
	if (distance(player->pos, vertical_hit) < distance(player->pos,
			horizontal_hit))
		return ((t_Collision){VERTICAL, vertical_hit});
	return ((t_Collision){HORIZONTAL, horizontal_hit});
}

t_DoublePair	collision_find_vertical_hit(t_mini_map *mini_map,
...
t_DoublePair	collision_find_horizontal_hit(t_mini_map *mini_map,
		t_character *player, double rayDir, t_IntPair direction)
{
	t_DoublePair	hit;
